...
import {
  AxiosClient,
  updateRequestAcceptService,
  updateRequestDeniedService,
  updateSettingAdminService,
} from '@/services';
import useSWR from 'swr';
import { GlobalContext, SettingNames } from '@/contexts';
import {
  AdminSetting,
  AdminSettingListData,
  HistoryTransaction,
...
const listTitleSetting = [
  {
    settingName: SettingNames.PostingFee,
    label: 'Phí đăng bài (VND)',
    isMoney: true,
  },
  {
    settingName: SettingNames.BookingFee,
    label: 'Phí hoa hồng (% theo giao dịch)',
  },
  {
    settingName: SettingNames.FreeNumberPost,
    label: 'Lượt miễn phí đăng bài/tháng',
  },
  {
    settingName: SettingNames.BoostPostFree,
    label: 'Phí đẩy bài',
    isMoney: true,
  },
...
const TableHistoryWallet: React.FC<TableHistoryWalletProps> = ({
  listItem,
  currentPage,
  itemsPerPage,
}) => {
      ...
      <tbody>
        {listItem.map((item, index) => {
          const statusObject = Status.find(
            (obj) => obj.statusEN.toLowerCase() === item.status.toLowerCase(),
          );
...
const AdminHome = () => {
  ...
  const { user, isLoadingModal, setIsLoadingModal, isLoading, setIsLoading } =
    useContext(GlobalContext) || {};

  const {
    data: listHistoryWallet,
    error,
    isLoading: loadingHistory,
  } = useSWR<HistoryTransaction>(
    user ? `/api/wallet/user/${user.id}/history` : null,
    fetcher,
    { refreshInterval: 10000 },
  );
  const { data: listSetting, mutate } = useSWR<AdminSetting>(
    `/api/Settings/get_listSetting`,
    fetcher,
    { refreshInterval: 10000 },
  );
  const { data: userWallet, isLoading: loadingWallet } = useSWR<WalletUserData>(
    user ? `/api/wallet/${user.id}/user_wallet` : null,
    fetcher,
    { refreshInterval: 5000 },
  );
  const { data: listIncoming, isLoading: loadingIncoming } =
    useSWR<ManagementReport>(
      dateRange.endDate && dateRange.startDate
        ? `/api/users/${encodeURIComponent(
            dateRange.startDate,
          )}&${encodeURIComponent(dateRange.endDate)}/report_income_Month`
        : null,
      fetcher,
      { refreshInterval: 10000 },
    );
  const { data: listUser, isLoading: loadingUser } = useSWR<ManageUser>(
    user ? `/api/users/managed/${user.id}` : null,
    fetcher,
    { refreshInterval: 10000 },
  );
  const { data: listProduct, isLoading: loadingProduct } = useSWR<ListProduct>(
    `/api/posts/GetListPost`,
    fetcher,
    { refreshInterval: 10000 },
  );
  const {
    ...
    mutate: reLoadRequest,
  } = useSWR<ListRequestWithdraw>(
    `/api/transactions/withdraw_request`,
    fetcher,
    { refreshInterval: 10000 },
  );

  const { register, handleSubmit, setValue, watch } =
    useForm<AdminSettingListData>({
      defaultValues: {
  ...
  const onSubmit = async (data: AdminSettingListData) => {
    ...
    for (const [settingName, settingValue] of Object.entries(dataAll)) {
      const setting = data.listSettingData.find(
        (item) => item.settingName === settingName,
      );
      ...
      if (settingValue !== undefined && setting) {
        if (
          settingName === SettingNames.PostingFee &&
          (settingValue < 0 || settingValue > 1000000)
        ) {
        ...
        if (
          settingName === SettingNames.FreeNumberPost &&
          (settingValue < 0 || settingValue > 10)
        ) {
  ...
  const filterHistoryWallet =
    listHistoryWallet &&
    listHistoryWallet.data &&
    listHistoryWallet.data.filter(
      (history) =>
        history.time &&
        history.time
          .trim()
  ...
  const pageCount = Math.ceil(
    filterHistoryWallet ? filterHistoryWallet.length / itemsPerPage : 0,
  );
  ...
  const visibleItems =
    filterHistoryWallet && filterHistoryWallet.length > 0
      ? filterHistoryWallet.slice(startIndex, endIndex)
      : [];
    ...
    <div className="relative flex flex-col px-6 py-10 gap-10">
      ...
      >
        ...
        <div className="flex gap-3 flex-col md:flex-row justify-end flex-wrap transition-all duration-500">
          <DownMetalBtn
            onClick={() => {
              if (listHistoryWallet) exportToExcel(listHistoryWallet.data);
            }}
          />
      ...
      {loadingHistory ? (
        ...
        !filterHistoryWallet ||
        listHistoryWallet.data.length === 0 ? (
