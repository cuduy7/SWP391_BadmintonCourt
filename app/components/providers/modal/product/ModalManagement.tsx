...
const ModalManagement = () => {
  ...
  const handleRadioChange = (dateSlot: string, slot: string) => {
    setSelectedSlot({ dateSlot, slot });
  };
  ...
  const {
    ...
    isLoading,
    mutate,
  } = useSWR<SlotData>(`/api/posts/${post_id}/status`, fetcher);
  ...
  const handleUpdateStatus = async () => {
    ...
    if (post_id) {
      const res = await UpdateStatusService({
        ...
        idUser: '0',
        postSlot: Array({
          dateSlot: selectedSlot.dateSlot,
          slot: Array(selectedSlot.slot),
        }),
      });
  ...
  return (
    ...
    >
      <form className="flex flex-col pb-5 gap-3 w-[60rem]">
        <label className="text-gray-600 font-semibold text-3xl">
          Quản lý sân
        </label>
        ...
        {isLoading ? (
          ...
          <div className="relative flex flex-col space-x-3 items-center justify-center h-96 text-primary-blue-cus">
            <p className="md:text-4xl text-3xl font-semibold">
              Đã xảy ra lỗi khi tải danh sách chỗ - error 500
            </p>
          ...
          <div className="flex flex-col p-4 gap-4 min-h-fit max-h-[40rem] overflow-auto">
              ...
              <div className="flex flex-col gap-2" key={index}>
                ...
                <div className="grid grid-cols-4 gap-3">
                  {item.slots.map((slot, slotIndex) => (
                    <div
                      className="col-span-1 grid grid-cols-4 gap-1 items-center w-40"
                      key={slotIndex}
                    >
                      <div className="col-span-1">
                        <input
                          type="radio"
                          checked={
                            selectedSlot?.dateSlot === item.dateSlot &&
                            selectedSlot?.slot === slot.content
                          }
                          onChange={() =>
                            handleRadioChange(item.dateSlot, slot.content)
                          }
