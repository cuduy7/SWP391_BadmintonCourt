...
const ProductDetail: React.FC<ProductDetailContentData> = ({
    ...
    const handleCheckboxChange = (dateSlot: string, slot: string, isChecked: boolean) => {
        ...
        setTotalPrice(prevPrice => {
            const selectedCount = Object.values(selectedSlots).flat().length + (isChecked ? 1 : -1);
        ...
        >
            ...
            <section className="relative font-semibold flex flex-col gap-2">
                ...
                >
                    ...
                    {postSlot && postSlot.map((item, index) => (
                        <option key={index} value={item.dateSlot}>{item.dateSlot}</option>
                    ))}
                </select>
            </section>
            {selectedDate && (
                <section className="relative font-semibold flex flex-col gap-2 mt-4">
                    ...
                    <div className="grid grid-cols-4 gap-3">
                        {postSlot && postSlot.find(item => item.dateSlot === selectedDate)?.slot.map((slot, slotIndex) => (
                            <div className="col-span-1 flex items-center gap-2" key={slotIndex}>
                                <input
                                    type="checkbox"
                                    checked={checkedStatus[`${selectedDate}-${slot}`] || false}
                                    onChange={(e) => handleCheckboxChange(selectedDate, slot, e.target.checked)}
            ...
            {!checkUser && (
                <section className="relative flex gap-3 font-semibold items-end">
                    ...
                    <span className="break-words text-primary-blue-cus text-xl">
                        {totalPrice ? formatMoney(new Decimal(totalPrice ? totalPrice : 0)) : 0}
                    </span>
                </section>
            )}
            {user && user.id === userId ? (
